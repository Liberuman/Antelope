...
import com.sxu.baselibrary.datasource.http.impl.listener.RequestListenerEx;
...
public class ResponseProcessor {
	...
	public static void process(ResponseBean response, RequestListener listener) {
		...
		if (listener instanceof SimpleRequestListener) {
			simpleRequestProcess(response, (SimpleRequestListener) listener);
		} else {
			requestProcess(response, (RequestListenerEx) listener);
		}
	}

	private static void simpleRequestProcess(ResponseBean response, SimpleRequestListener listener) {
		if (listener == null) {
			return;
		}
		if (response != null) {
	...
	private static void requestProcess(ResponseBean response, RequestListenerEx listener) {
		if (response != null) {
			if (response.code == RESPONSE_CODE_SUCCESS) {
