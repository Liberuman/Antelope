...
import com.sxu.basecomponent.application.BaseApplication;
 ...
 *
 * 说明：1. 对于单个或少量数据, 提倡使用putXXX保存数据,如果保存多个数据,提倡使用
 *          putXXXNoCommit进行保存, 最后调用commit进行持久化；
 *      2. base模块中其实提供了KV存储类，此模块使用单独的SP为了避免层级依赖；
 *
 * Author: Freeman
 *
 * Date: 2018/6/20
...
public class PreferencesManager {
    ...
    static {
        init(BaseApplication.context, PREFERENCE_NAME);
    }
    ...
    public static void init(Context context, String preferenceName) {
        if (context == null) {
            throw new NullPointerException("Please inherit BaseApplication or call init in Application's onCreate");
        }
