...
public class SwipeMenuLayout extends ViewGroup {
     ...
     * 当前是否正在关闭其他Item
     */
    private boolean closingOtherItem = false;
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView.getVisibility() != GONE) {
                ...
                if (i > 0) {//第一个布局是Left item，从第二个开始才是RightMenu
                    ...
                    contentWidth = Math.max(childView.getMeasuredWidth(), maxWidth);
                    // 解决内容布局不占满的问题
                    measureChildWithMargins(childView, MeasureSpec.makeMeasureSpec(getScreenWidth(),
                            MeasureSpec.EXACTLY), 0, heightMeasureSpec, 0);
                }
            }
        }
    ...
    protected void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
        ...
        int horizontalPadding = getPaddingLeft() + getPaddingRight() + widthUsed;
        int verticalPadding = getPaddingTop() + getPaddingBottom() + heightUsed;
        if (lp instanceof MarginLayoutParams) {
            MarginLayoutParams layoutParams = (MarginLayoutParams) lp;
            horizontalPadding += layoutParams.leftMargin + layoutParams.rightMargin;
            verticalPadding += layoutParams.topMargin + layoutParams.bottomMargin;
        }
        final int childWidthMeasureSpec = getChildMeasureSpec(parentWidthMeasureSpec,
                horizontalPadding, lp.width);
        final int childHeightMeasureSpec = getChildMeasureSpec(parentHeightMeasureSpec,
                verticalPadding, lp.height);
...
//                    }
                    ...
                    if (mViewCache != null) {
                        ...
                        closingOtherItem = true;
                    } else {
                        closingOtherItem = false;
                    }
...
/*                    if (!mScroller.isFinished()) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    // 如果是滑动行为或当前已有展开的Item，则不继续分发事件
                    if (isUserSwiped || (mViewCache != null && ev.getRawX() < (screenWidth - mRightMenuWidths))) {
                        if (!isUserSwiped && mViewCache != null) {
                            mViewCache.smoothClose();
                        }
                        return true;
                    }
